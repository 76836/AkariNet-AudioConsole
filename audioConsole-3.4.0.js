...
export class AkarinetVoice extends EventTarget {
        ...
        try {
            ...
            if (this.config.wakeChimeURL) {
                try {
                    ...
                    this._log('WARN', `Wake chime disabled: ${e.message}`);
                    this.wakeChimeAudio = null;
                }
            }
    ...
    async _handleSpeech(audio) {
        ...
        const startDeltaMs = hasWakeSound
            ? (this.speechStartTime - this.wakeSoundDetectedTime)
            : Infinity;
        // Match wake sound and speech start if they are close in time, regardless of order.
        // This preserves support for models that fire wake detection slightly after speech has begun.
        const inSession = hasWakeSound && Math.abs(startDeltaMs) <= this.config.wakesoundDelay;
